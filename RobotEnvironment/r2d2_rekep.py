...
## 6D位姿的坐标转换方法，供外部调用
# 6D位姿的坐标转换
def pose6d_world2robot_from_rekep(pose6d):

        """
        将[x,y,z,rx,ry,rz]从世界/相机坐标系转换为机器人坐标系
        """

        # vec2mat
        xyz = pose6d[0:3]# 位置
        orientation_euler = pose6d[3:]# 笛卡尔表示的方向

        mat_pose_world = np.eye(4) # 4*4的单位阵

        mat_pose_world[:3,3] = xyz # 位置嵌入到矩阵中
        mat_pose_world[:3,:3] = R.from_euler('xyz', orientation_euler).as_matrix() # 方向转换为旋转矩阵嵌入到矩阵中
        
        '''
        在三维空间中，四元数、欧拉角（或旋转向量）的本质都是 SO(3) 群中的元素，它们只不过是同一旋转的不同参数化方式。
        只要你用相应的转换函数（如 SciPy 的 R.from_quat、R.from_euler 或 R.from_rotvec）将它们转换为 3×3 旋转矩阵
        这三个表示完全一致，都会得到同一个矩阵。
        '''
        ...
        # 将相机坐标系下的位姿转换为机器人坐标系下的位姿
        # world指相机坐标系
        mat_pose_robot = transformation_matrix@mat_pose_world

        # mat2vec
        robot_xyz = mat_pose_robot[:3,3]
        robot_euler = R.from_matrix(mat_pose_robot[:3,:3]).as_euler('xyz')
        robot_pose6d = np.hstack((robot_xyz, robot_euler))

        # 打印DEBUG
        print('DEBUG: robot_pose6d_before_transformation:',pose6d)
        print('DEBUG: robot_position_before_transformation:',xyz)
        print('DEBUG: robot_orientation_before_transformation:',orientation_euler)
        print('DEBUG: robot_mat_before_transformation:',mat_pose_world)

        print('DEBUG: transformation_matrix:',transformation_matrix)

        print('DEBUG: robot_pose6d_after_transformation:',robot_pose6d)
        print('DEBUG: robot_position_after_transformation:',robot_xyz)
        print('DEBUG: robot_orientation_after_transformation:',robot_euler)
        print('DEBUG: robot_mat_after_transformation:',mat_pose_robot)
...
## 7D位姿的坐标转换方法，供外部调用
# 7D位姿的坐标转换
def pose7d_world2robot_from_rekep(pose7d):
...
class MainR2D2:
    ...
    # 6D位姿的坐标转换
    def pose6d_world2robot(self,pose6d):
